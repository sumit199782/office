(() => {
    ...
    function appendInputRow(viewport, value) {
        ...
        var viewportOptions = viewports.filter(function (viewportName) {
            return finalValue[viewportName] <= 0 || viewport === viewportName;
        ...
        <div class="slds-grid viewport-row">
            ...
            <div class="slds-col slds-p-left_medium slds-p-right_medium">
                <div class="slds-form-element slds-form-element_horizontal slds-is-editing">
                    <label class="slds-form-element__label" for="small">${configuration.labels.value}</label>
                    <div class="slds-form-element__control">
                        <input type="text" required class="slds-input input-value" min="0" value="${value}" />
                    </div>
                </div>
            </div>
     ...
     */
    function initializeDevices(selectedValue) {
        if (!selectedValue) {
            return;
        }

        selectedValue.split('|').forEach(function (value, idx) {
            appendInputRow(viewports[idx], value);
        })
    }
    ...
    function saveValues() {
        ...
        [].forEach.call(rootEditorElement.querySelectorAll('.viewport-row'), function (row) {
            ...
            var value = row.querySelector('.input-value').value;

            if (!viewport || !value) {
                return;
            }

            finalValue[viewport] = value;
        });
    }
    ...
    listen('sfcc:ready', async ({ value, config, isDisabled, isRequired, dataLocale, displayLocale }) => {
        ...
        init(isRequired);
        initializeDevices(selectedValue);
        ensureDeviceLimits();
        ...
        rootEditorElement.querySelector('.save-viewport').addEventListener('click', event => {
            ...
            emit({
                type: 'sfcc:value',
                payload: {
                    value: Object.keys(finalValue).map(function (key) {
