...
function setURL(svc, endpoint, parameters, credPrefix) {
   ...
   */
    ...
    svc.setCredentialID(
        credPrefix + CurrentSite.getCustomPreferenceValue('subproAPICredSuffix')
    );
    ...
    'subpro.http.get.subscriptions',
    {
        ...
        createRequest: function (svc, args) {
            ...
            setURL(
                svc,
                'subscriptions.json',
                'customer_id=' + Encoding.toURI(args.customer_id)
            );
        },
    ...
    'subpro.http.get.customers',
    {
        ...
        createRequest: function (svc, args) {
            ...
            if (args.customer_id) {
                setURL(
                    svc,
                    'customers/' + Encoding.toURI(args.customer_id) + '.json',
                    ''
                );
            } else if (args.email) {
...
module.exports.SubproHttpGetToken = LocalServiceRegistry.createService(
    'subpro.http.get.token',
    {
        ...
        createRequest: function (svc, args) {
          ...
          + args.scope
          + '&customer_id='
          + Encoding.toURI(args.customer_id),
    ...
    'subpro.http.get.paymentprofile',
    {
        ...
        createRequest: function (svc, args) {
            ...
            if (args.paymentprofile_id) {
                setURL(
                    svc,
                    'vault/paymentprofiles/'
            + Encoding.toURI(args.paymentprofile_id)
            + '.json',
                    ''
                );
                ...
                setURL(
                    svc,
                    'vault/paymentprofiles.json',
                    'transaction_id=' + Encoding.toURI(args.transaction_id)
                );
            } else {
                throw new Error(
                    'subpro.http.get.paymentprofile requires a paymentprofile_id or transaction_id'
                );
            }
        },
